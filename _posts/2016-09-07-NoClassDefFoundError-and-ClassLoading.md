...
2. 某工程的某个类由于执行类构造器`<clinit>()`失败而导致类加载失败。其原始异常栈大致如下：

    ```java
    严重: Servlet.service() for servlet [appServlet] in context with path [/ms] threw exception [Handler processing failed; nested exception is java.lang.ExceptionInInitializerError] with root cause
